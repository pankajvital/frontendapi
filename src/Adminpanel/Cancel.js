...
const Cancel = () => {
  ...
  useEffect(() => {
    ...
    axios.get('${urlbc}/bookings')
      .then(response => {
        ...
        const redwBookings = response.data.filter(booking => booking.bookingColor === 'red');
        
        // Update state with fetched red bookings data
        setBookingData(redwBookings);
  
        // Set colors based only on fetched red bookings data
        const colors = {};
        redwBookings.forEach((booking, index) => {
          colors[index] = 'red'; // Set color to yellow for yellow bookings
        });
  ...
  useEffect(() => {
    ...
    const apiUrl = "${urlbc}/bookings/cancel";
  ...
  useEffect(() => {
    document.title = 'Canceelled Page'; // Set your dynamic title here
  }, []);
