...
const White = () => {
  ...
  useEffect(() => {
    ...
    axios.get('${urlbc}/bookings')
      .then(response => {
        ...
        const whiteBookings = response.data.filter(booking => booking.bookingColor === 'white');
        
        // Update state with fetched white bookings data
        setBookingData(whiteBookings);
  
        // Set colors based only on fetched white bookings data
        const colors = {};
        whiteBookings.forEach((booking, index) => {
          colors[index] = 'white'; // Set color to yellow for yellow bookings
        });
  ...
  useEffect(() => {
    ...
    const apiUrl = "${urlbc}/bookings/white";
