  ...
  useEffect(() => {
    ...
    axios.get('${urlbc}/bookings')
      .then(response => {
        ...
        const greenwBookings = response.data.filter(booking => booking.bookingColor === 'green');
        
        // Update state with fetched green bookings data
        setBookingData(greenwBookings);
  
        // Set colors based only on fetched green bookings data
        const colors = {};
        greenwBookings.forEach((booking, index) => {
          colors[index] = 'green'; // Set color to yellow for yellow bookings
        });
